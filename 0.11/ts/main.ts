...
function getContent(mode: number): DocumentFragment {
  ...
  if (mode == 0) {
    ...
    let todoGroup = getGroup(mode, { id: -1, title: '正在进行' }, todoList).render()
    let doneGroup = getGroup(mode, { id: -1, title: '已完成' }, doneList).render()
  ...
  if (mode == 1) {
    ...
    data.groups.forEach((group) => {
      //   console.log(group)
      let tempGroup = getGroup(mode, group, groupedItems[group.id]).render()
      fragment.appendChild(tempGroup)
    })
    let tempGroup = getGroup(mode, { id: -1, title: '未分组' }, groupedItems['default']).render()
    fragment.appendChild(tempGroup)
  }
  return fragment
}
...
function render(mode) {
  ...
  if (mode == 0) addGroup.setAttribute('style', 'display:none;')
  else addGroup.setAttribute('style', '')
