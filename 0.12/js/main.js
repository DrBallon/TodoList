...
function getContent(mode) {
    ...
    var inner = new VDom('div', { class: 'inner' }, []);
    //分组
    if (mode == 0) {
        var todoList_1 = [];
        var doneList_1 = [];
        data.list.forEach(function (item) { return (item.done ? doneList_1.push(item) : todoList_1.push(item)); });
        var todoGroup = getGroup(mode, { id: -1, title: '正在进行' }, todoList_1);
        var doneGroup = getGroup(mode, { id: -1, title: '已完成' }, doneList_1);
    ...
    if (mode == 1) {
        ...
        data.groups.forEach(function (group) {
            //   console.log(group)
            var tempGroup = getGroup(mode, group, groupedItems_1[group.id]);
            inner.addChild(tempGroup);
        });
        var tempGroup = getGroup(mode, { id: -1, title: '未分组' }, groupedItems_1['default']);
        inner.addChild(tempGroup);
    }
...
function getContentHeight() {
    function getHeight(ele) {
        var heightStr = window.getComputedStyle(ele, null).height;
        var heightNum = heightStr.slice(0, heightStr.indexOf('px'));
        return parseInt(heightNum);
    }
    var switchBtns = document.querySelector('#switch-btns');
    ...
    var height = getHeight(body) - getHeight(header) - getHeight(switchBtns) - getHeight(footer);
    return height;
}
//渲染主函数
function render(mode) {
    ...
    var miniScroll = new MiniSCroll(inner, content, null, null);
...
window.onload = function () {
    ...
    addTodo.addEventListener('blur', function (e) {
