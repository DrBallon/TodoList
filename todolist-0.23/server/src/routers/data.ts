...
const testUserId = new ObjectID('5f7710672ae7c270cd646650');
...
async function getData(req: Request, res: Response) {
  ...
  const items: Item[] = await ModelItem.aggregate()
    ...
    .exec();
  const groups: Group[] = await ModelGroup.aggregate().project({ id: '$_id', title: 1, _id: 0 }).exec();

  const curMode = await User.findOne({ _id: testUserId }).exec();
